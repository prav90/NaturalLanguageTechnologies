/*
 ...
 Program Modified by Raj Praveen for Natural Language Technologies Homework
 
 Added Additional Rules in Step4() method of this program
 to deal with Suffixes "er", "est" and "an"
 
 The source code is from http://tartarus.org/martin/PorterStemmer/java.txt, I have only made a few changes as mentioned above.
...
public class PorterStemmer {
	...
	private final void step4() {
		...
		case 'l':
			...
			break;
		/* Added new rule to match to deal with "ian"  
	     * replace all such occurrences in the end with null
		*/
		case 'n':
			if(ends("ian")) {
				r("");
		...
		case 's':
			...
			break;
		/* Added new rule to match to deal with - most of the words end with tier, ier and er 
		 * we replace tier with t, ier and er with null
		 * we then check for a double consonant at the end of the word
		 * dd, gg,tt etc and convert it to d, g, t 
		 */
		case 'r':
			if(ends("tier")) {
				r("t");
			...
			break;
		/* end added rule */
		/* Added new rule to match to deal with "est" - most of the words end with liest, est, iest,  
		 * replace all such occurrences in the end with null
	     * we then check for a double consonant at the end of the word
	     * dd, gg,tt etc and convert it to d, g, t 
	    */
		case 't':
			if(ends("liest") || ends("iest") || ends("est")) {
				r("");
			}
			if(doublec(k))
				k -= 1;
	...
	public static void main(String[] args) {
		...
		if(args.length == 0)
			System.out.println("Error : Pass a file with text as an argument");
